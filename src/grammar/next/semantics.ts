...
export type Handler<T> = (ctx: Context) => T;
...
const parseId = ({ name, loc }: $$.Id | $$.TypeId): Handler<A.AstId> => (ctx) => {
    if (name.startsWith("__gen")) {
        ctx.err.noGenVariable(loc);
    }
    if (name.startsWith("__tact")) {
        ctx.err.noTactVariable(loc);
    }
...
const parseFunctionAttribute = ({ name, loc }: $$.FunctionAttribute): Handler<A.AstFunctionAttribute> => (ctx) => {
    return ctx.ast.FunctionAttribute(name, loc);
};
...
const parseFunctionAttributes = (nodes: readonly $$.FunctionAttribute[], isAbstract: boolean, loc: $.Location): Handler<A.AstFunctionAttribute[]> => (ctx) => {
    ...
    for (const { name, loc } of nodes) {
        if (k.has(name)) {
            ctx.err.noDuplicateFunctionAttribute(name, loc);
        }
        k.add(name);
    }
    if (isAbstract && !k.has("abstract")) {
        ctx.err.mustBeAbstractFunction(loc);
    }
    if (!isAbstract && k.has("abstract")) {
        ctx.err.cantBeAbstractFunction(loc);
    }
...
const parseConstantAttributes = (nodes: readonly $$.ConstantAttribute[], isAbstract: boolean, loc: $.Location): Handler<A.AstConstantAttribute[]> => (ctx) => {
    ...
    for (const { name, loc } of nodes) {
        if (k.has(name)) {
            ctx.err.noDuplicateConstantAttribute(name, loc);
        }
        k.add(name);
    }
    if (isAbstract && !k.has("abstract")) {
        ctx.err.mustBeAbstractConstant(loc);
    }
    if (!isAbstract && k.has("abstract")) {
        ctx.err.cantBeAbstractConstant(loc);
    }
...
const parseConstantDefInModule = (node: $$.Constant): Handler<A.AstConstantDef> => (ctx) => {
    ...
    if (typeof firstAttribute !== 'undefined') {
        ctx.err.noModuleConstantAttributes(node.loc);
        result.attributes = [];
    }
    return result;
};
...
const parseImport = ({ path, loc }: $$.Import): Handler<A.AstImport> => (ctx) => {
    if (path.value.includes("\\")) {
        ctx.err.noBackslashInImport(loc);
...
export const parseModule = ({ imports, items }: $$.Module): Handler<A.AstModule> => (ctx) => {
    return ctx.ast.Module(
        map(imports, parseImport)(ctx),
        map(items, parseModuleItem)(ctx), // astOfModuleItem
    );
};